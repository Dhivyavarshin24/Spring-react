...
export default function App() {
  ...
  const editBook = (book) => {
    ...
    setForm({ title: book.title, author: book.author, price: book.price, publicationYear: book.publicationYear });
  };
    ...
    <div style={{ padding: "2rem", fontFamily: "sans-serif" }}>
      ...
      <form onSubmit={saveBook} style={{ marginBottom: "1rem" }}>
        ...
        <input
          ...
          onChange={(e) => setForm({ ...form, publicationYear: Number(e.target.value) })}
        />
        <button type="submit">{editingId == null ? "Add" : "Update"}</button>
        {editingId != null && (
          <button type="button" onClick={() => { setEditingId(null); setForm({ title: "", author: "", price: 0, publicationYear: 2000 }); }}>
            Cancel
          </button>
        )}
      </form>
      ...
      <table border="1" cellPadding="6" style={{ borderCollapse: "collapse", width: "100%" }}>
        <thead>
          <tr>
            <th>ID</th><th>Title</th><th>Author</th><th>Price</th><th>Year</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
          {books.map((b) => (
